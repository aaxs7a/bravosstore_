...
/*
  ...
  - product: dados do produto vindos do Supabase.
...
import type { Product } from "../_lib/bravosSupabase";
  ...
  return (
    ...
    <div className="group bg-zinc-900/20 border border-zinc-800/80 hover:border-[#00ff66]/30 transition-all duration-300 rounded-2xl flex flex-col overflow-hidden h-full hover:scale-[1.02] hover:shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
      ...
      <Link
        ...
        className="relative block aspect-square w-full overflow-hidden bg-zinc-950 cursor-pointer"
      >
        <img
          src={product.image}
          alt={product.name}
      ...
      <div className="p-4 flex flex-col flex-grow justify-between space-y-4">
        <div className="space-y-1">
          ...
          <Link
            href={`/produto/${product.id}`}
            className="text-sm font-black text-white hover:text-[#00ff66] transition-colors line-clamp-2 uppercase tracking-wide cursor-pointer h-10 block"
          >
            {product.name}
          </Link>
        </div>
